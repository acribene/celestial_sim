...
void Quadtree::render() const
{
    for (const auto& node : m_nodes) {
        if( !node.isEmpty() ) {
            ...
            float screenX = GetScreenWidth() / 2.0f + static_cast<float>(node.quad.center.getX() * SCALE);
            float screenY = GetScreenHeight() / 2.0f + static_cast<float>(node.quad.center.getY() * SCALE);
